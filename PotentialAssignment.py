...
def potential_map_gen(matrix, dest):
    ...
    # Filling potential_matrix to have same num of elements as matrix
    ...
    for i in Q:
        if i in OPEN:
            # Remove obstacle nodes from OPEN (OPEN = [all - obstacles])
            OPEN.remove(i)
    #print("\nAfter assigning Eo to obstacles: ")
    #print("OPEN = ", OPEN)
    ...
    for i in Q:
        ...
        # Remove dest node from OPEN (OPEN = [all - obstacles - dest])
        OPEN.remove(i)
    #print("\nAfter assigning Eo to dest: ")
    #print("OPEN = ", OPEN)
    #print("CLOSE = ", CLOSE)
    #print("TEMP = ", TEMP)
    #print("Q = ", Q)
    ...
    def N_loop(TEMP, CLOSE, OPEN, Pq, Eo, Q):
        ...
        for i in N:
            if i in OPEN:
                N_rem.append(i)
        N = N_rem
        #print("Neighbours without Eo: ")
        #print("N = ", N)

        for i in N:
            Pq.append(Eo-10)  # Add Eo corresponding to each neighbour in Pq

        # Neighbour assigned Eo so TEMP[0] is CLOSE (CLOSE = [obstacles + TEMP[0]])
        CLOSE.append(TEMP[0])
...
# Destination input
'''d = input("Enter destination as i,j: ")
dest = d.split(",")
dest[0] = int(dest[0])
dest[1] = int(dest[1])'''
dest = [len(matrix) - 1, len(matrix) - 1]
print("dest = ", dest)

# Calling potential_map_gen function
potential_matrix = potential_map_gen(matrix, dest)
print("Potential Matrix generated!!!")
